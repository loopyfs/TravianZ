...
tz_def('ANL_ROMAN_DESC_1', 'Grâce à leur haut niveau de développement social et technologique, les Romains excellent dans la construction et son organisation. Leurs troupes font également partie de l’élite de Travian. Elles sont très équilibrées et efficaces aussi bien en attaque qu’en défense.');
...
tz_def('ANL_TRIBES_INTRO', 'Vous pouvez choisir entre trois peuples dans Travian : les Romains, les Gaulois et les Germains. Chaque peuple possède ses avantages et ses inconvénients, et leurs troupes sont très différentes. Il est donc important de choisir celui qui correspond le mieux à votre style de jeu.');
...
tz_def('ANL_TEUTON_PLUNDER_BONUS', 'Bonus de pillage : les cachettes ennemies ne protègent que 66 % de leur capacité normale dans Travian 2.5 et 3, et 80 % dans Travian 3.5.');
...
tz_def('ANL_TEUTON_CRANNY', 'Germains : les cachettes ennemies ne protègent que 66 % de leur capacité normale dans Travian 2.5 et 3, et 80 % dans Travian 3.5.');
...
tz_def('ANL_ROMAN_TRADE_OFFICE', 'Romains : dans Travian 3.5, le comptoir de commerce romain augmente la capacité de transport de 20 % par niveau.');
...
tz_def('PUBLIC_UNOFFICIAL_PROJECT', 'Ce projet non officiel est réalisé par des fans et n’est pas affilié à Travian Games GmbH.');
...
tz_def('PUBLIC_TERMS_PROJECT', 'TravianZ est un projet open source réalisé par des fans et créé par Shadow.');
...
tz_def('PUBLIC_TERMS_NO_AFFILIATION', 'Ce projet n’est pas affilié à Travian Games.');
...
tz_def('PUBLIC_RULES_INTRO_1', 'Les règles ci-dessous complètent les conditions générales de Travian. Vous devez également consulter ces conditions afin de vérifier ce qui est autorisé, notamment lorsqu’un compte a été suspendu pour infraction aux règles.');
tz_def('PUBLIC_RULES_INTRO_2', 'Il est interdit d’inciter, de manipuler, d’encourager, d’aider ou de s’entendre avec d’autres personnes pour enfreindre les règles de Travian. Ces règles s’appliquent sans exception, y compris aux joueurs qui prévoient de supprimer leur compte ou dont le compte est en cours de suppression.');
...
tz_def('PUBLIC_RULES_PASSWORD_DAMAGE', 'Aucune compensation n’est accordée pour les dommages causés par une personne connaissant le mot de passe d’un compte. Toute personne recevant ce mot de passe est soumise aux règles de Travian ainsi qu’aux conditions générales.');
...
tz_def('PUBLIC_RULES_SAME_WORLD_TRANSFER', 'Pour échanger un compte avec une personne jouant sur le même monde, les deux personnes doivent envoyer un message à admin@travian.com depuis l’adresse électronique actuellement associée au compte. Le message doit contenir les informations suivantes :');
...
tz_def('PUBLIC_RULES_REAL_MONEY', 'Toute vente ou tout achat contre de l’argent réel concernant des comptes, unités, villages, ressources, services ou tout autre élément de Travian est interdit. La vente de comptes Travian, ainsi que tout transfert indirect, même à titre gratuit, lié à des sites d’enchères ou à d’autres transactions financières, est également interdite.');
...
tz_def('PUBLIC_RULES_BLACKMAIL', 'Faire chanter des joueurs d’une manière contraire aux règles de Travian ou aux conditions générales.');
tz_def('PUBLIC_RULES_ADVERTISING', 'Toute publicité n’ayant pas été autorisée par l’équipe Travian est interdite.');
...
tz_def('PUBLIC_RULES_NO_REPLACEMENT', 'Les ressources, bâtiments, villages ou troupes perdus pendant une suspension ne sont pas considérés comme faisant partie de la sanction et ne seront pas remplacés par l’équipe Travian. Aucun joueur ne peut réclamer le remboursement ou le remplacement du temps Plus/Or perdu pendant la suspension.');
tz_def('PUBLIC_RULES_NO_SPECIAL_TREATMENT', 'Les utilisateurs de Travian Plus/Or ne bénéficient d’aucun traitement particulier concernant les règles du jeu, ni dans le délai de traitement du dossier ni dans la sanction.');
...
tz_def('PUBLIC_RULES_OWNER_INFORMATION', 'L’équipe Travian ne communiquera par ailleurs aucune information à une personne autre que le propriétaire du compte.');
...
tz_def('PUBLIC_RULES_CHANGE_ANY_TIME', 'L’équipe Travian se réserve le droit de modifier les règles à tout moment.');
...
tz_def('ADM_MANUAL_ON_NATURE_AND_NATARS', 'Manuel sur la Nature et les Natars');
tz_def('ADM_ENABLE_DISABLE_MANUAL_INFO', 'Activer (désactiver) les infos du manuel');
tz_def('ADM_ENABLE_DISABLE_DISPLAYING_INFORMATION_IN_THE', 'Activer (désactiver) l\'affichage dans le manuel des informations sur les troupes de la Nature et des Natars');
...
tz_def('ADM_IF_ENABLED_THEN_THE_DIRECT_LINKS_WILL_BE_PLA', 'Si activé, les liens directs sont placés dans le menu de gauche ; si désactivé, dans le menu de droite comme dans le Travian original');
...
tz_def('ADM_YES', 'Oui');
tz_def('ADM_NO', 'Non');
tz_def('ADM_TRAVIAN_OFFICIAL', 'Travian officiel');
tz_def('ADM_TRAVIANZ_EXTENDED', 'TravianZ étendu');
...
tz_def('ADM_ACTIVATE_TRAVIAN_PLUS_FOR_ALL_PLAYERS_ON_THE', 'Activer Travian Plus pour TOUS les joueurs du serveur.');
...
tz_def('ADM_RETURN_TO_NATARS', 'Rendre aux Natars');
...
tz_def('ADM_SECURE_ACCESS_TO_TRAVIANZ_PANEL', 'Accès sécurisé au panneau TravianZ');
...
tz_def('ADM_THIS_ACTION_WILL_DISABLE_TRAVIAN_PLUS_FOR_AL', 'Cette action désactivera Travian Plus pour TOUS les joueurs.');
...
tz_def('ADM_USE_IT_TO_PRIORITISE_WHICH_PAIRS_A_HUMAN_SHO', '&mdash; utilisez-le pour prioriser les paires qu\'un humain doit examiner. Rien n\'est banni automatiquement.');
